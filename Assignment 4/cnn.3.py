#Here we try using 2 convolutional layers, 3x3x3x32 and 3x3x32x64, along with two fully connected layers with 625, and 128 neurons
...
def model(X, w, w2, w_fc, w_fc2, w_o, p_keep_conv, p_keep_hidden):
    ...
    l5 = tf.nn.relu(tf.matmul(l4, w_fc2))
...
with tf.name_scope("Weights"):
    ...
    w_fc = init_weights([64 * 8 * 8, 625]) # FC 32 * 16 * 16 inputs, 625 outputs
    w_fc2 = init_weights([625, 128])
...
with tf.name_scope("model"):
    py_x = model(X, w, w2, w_fc, w_fc2, w_o, p_keep_conv, p_keep_hidden)
...
with tf.Session() as sess:
    ...
    writer = tf.summary.FileWriter('./board/question1part5/model4/', sess.graph)
