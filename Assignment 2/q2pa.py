...
# helper.printImg(np.asarray(cleanData[0:1, :])[0])
# print("")
# helper.printImg(np.asarray(noisyData[6][0:1, :])[0])

##########################
## BUILD the Neural net ##
##########################
model = []
for netNum in range(0, 5):#for each net with hidden neuron numbers 5, 15, ... 25
    options.layerSizes[1] = 5 + netNum*5
...
results = []
for netNum in range(0, 5):
    print ("\n\nEvaluating Neural net number: ", netNum, " Hidden Nodes: ", (5+(netNum*5)))
    results.append([])
    for i in range (0, 7): ## for all levels of noise
...
for netNum in range(0, 5):
    ...
    for noiseLvl in range (0, 7):
        print ("Results for Noise level: ", noiseLvl/2)
...
for netNum in range(0, 5):
    x.append([])
    y.append([])
    l.append(str(5 + (netNum*5)) + " hidden neurons")
    for noiseLvl in range(0, 7):
